...
class AccountBillingDashboard(models.Model):
    ...
    def _count(self):
        if self.content_type == 'col_day':
            ...
            total_count = 0
            invoices_billing = self.env['account.invoice'].sudo().search([('billing_id','!=',False), ('payment_ids.payment_date','>=',datetime.today().date().replace(day=1)), ('payment_ids.payment_date','<',datetime.today().date().replace(day=1)+relativedelta(months=1))])
            for invoice_billing in invoices_billing:
    ...
    def get_kanban_dashboard_bar_datas(self):
        if self.content_type == 'col_mo':
            ...
            for i in range(1,num_days+1):
                ...
                invoices_billing = self.env['account.invoice'].sudo().search([('billing_id','!=',False), ('payment_ids.payment_date','=',get_date_start)])
                count = 0
                for invoice_billing in invoices_billing:
    ...
    content_type = fields.Selection([('col_day','Daily Collection'), ('col_mo','Daily Collection')], string="Content Type")
