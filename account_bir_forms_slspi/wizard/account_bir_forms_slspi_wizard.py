...
class AccountBirFormsSlspi(models.TransientModel):
    _name = "account.bir.forms.slspi"
...
#    sch1_ids = fields.One2many('bir.generate.dat.sch1','dat_id',string="Schedule 1")
    sch1_ids = fields.Many2many('account.bir.forms.slspi.sch1',string="Schedule 1")
...
#                'dat_id': self.id,
            ...
            self.sudo().write({'sch1_ids': [[4,self.env['account.bir.forms.slspi.sch1'].sudo().create(sch1_vals).id]]})
    ...
    def action_generate(self):
        ...
        path = str("/opt/odoo11/odoo-custom-addons/account_bir_forms_slspi/dat/")
        if self.dat_type == 'sales':
            letter = 'S'
        elif self.dat_type == 'purchases':
            letter = 'P'
...
class AccountBirFormsSlspiSch1(models.TransientModel):
    _name = "account.bir.forms.slspi.sch1"
